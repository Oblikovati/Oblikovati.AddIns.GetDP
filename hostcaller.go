...
// cgoHostCaller implements getdp.HostCaller over the C-ABI host callbacks stored on
...
func (cgoHostCaller) Call(method string, req []byte) ([]byte, error) {
	...
	mu.Unlock()
	if call == nil {
		return nil, errors.New("getdp: host not connected")
	}
	...
	if resp == nil {
		...
		return nil, errors.New("getdp: host call failed")
	}
