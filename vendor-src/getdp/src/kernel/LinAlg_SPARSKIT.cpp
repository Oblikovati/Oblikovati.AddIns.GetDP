...
// Oblikovati patch (vendor-src/getdp/NOTICE.md): the 3.5.0 release tarball still
// carries the pre-3.5 four-argument signature here, while LinAlg.h and every caller
// use the five-argument form with `bool silent` — the SPARSKIT build fails to link
// out of the box. The parameter only mutes an informational print in the PETSc
// variant; this variant prints nothing, so it is accepted and ignored.
