...
// Package gplpurity holds the add-in license-boundary guard (same pattern as the
// CalculiX add-in, Oblikovati#1614 audit B3). It is test-only: the guard runs in the
	...
	for _, dep := range strings.Fields(goOutput(t, "list", "-deps", "./...")) {
		if isOblikovati(dep) && !inModule(dep, self) && !inModule(dep, "oblikovati.org/api") {
			t.Errorf("shipped (non-test) code depends on %q — an add-in links ONLY the Apache-2.0 "+
				"api module; reach the host over api/client instead (ADR-0016/0018).", dep)
		}
	}
}
